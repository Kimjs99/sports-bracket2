...
import { ChevronLeft, Bell, Trophy, Swords, Trash2, Plus, Search, CalendarDays, MapPin, Clock, Lock } from 'lucide-react';
...
function getFinalRankings(rounds) {
  ...
  if (rounds.length >= 2) {
    ...
    const semiLosers = semiRound?.matches
          ...
          <div className="bg-blue-600 text-white rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-blue-200 mt-0.5">
                전체 {teamMatches.length}경기
                {byeMatches.length > 0 && ` (부전승 ${byeMatches.length}개 포함)`}
              </div>
            </div>
...
export default function Dashboard() {
  ...
  const totalNonBye = rounds.flatMap(r => r.matches).filter(m => !m.isBye).length;
  const completedCount = doneMatches.length;
  const isTournamentOver = !!winner;

  function addNotice() {
    if (!noticeTitle.trim() || !noticeContent.trim()) return;
    dispatch({ type: ACTIONS.ADD_NOTICE, payload: { title: noticeTitle.trim(), content: noticeContent.trim() } });
    setNoticeTitle('');
    setNoticeContent('');
  }
  ...
  const tabs = [
    ...
    { id: 'notices', label: `공지 (${notices.length})` },
  ];
          ...
          {tabs.map(t => (
            <button
              ...
              className={`px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap transition-colors flex items-center gap-1.5
                ${activeTab === t.id ? 'border-white text-white' : 'border-transparent text-blue-300 hover:text-white'}`}
            >
              {t.id === 'notices' && !isLoggedIn && <Lock size={10} />}
              {t.label}
            </button>
          ))}
        </div>
            ...
            {isTournamentOver ? (
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <StatCard label="완료 경기" value={`${completedCount}/${totalNonBye}`} color="green" />
                  <StatCard label="부전승" value={`${meta.byeCount}개`} color="orange" />
                </div>
                <div className="mt-4">
                  <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                    <span>진행률</span>
                    <span>{totalNonBye > 0 ? Math.round((completedCount / totalNonBye) * 100) : 0}%</span>
                  </div>
                  <div className="h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-blue-500 rounded-full transition-all duration-500"
                      style={{ width: `${totalNonBye > 0 ? (completedCount / totalNonBye) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              </div>
                  ...
                  return (
                    <div key={r.roundNum}>
                      ...
                      <div className="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? 'bg-green-500' : 'bg-blue-400'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
              ...
              notices.map(n => (
                <div key={n.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {isLoggedIn && (
                      <button
                        onClick={() => dispatch({ type: ACTIONS.DELETE_NOTICE, payload: { id: n.id } })}
