...
### 🎉 정식 릴리스
- 실사용 검증 완료(화접중학교 학교스포츠클럽 축제 — 학년·성별 대진 9건 운영)에 따라 v1.0.0 승격
- 주요 기능: 멀티 테넌트(등록 코드 게이트) · 토너먼트/리그전/조별리그 · 자동 추첨/수동 배정 · 게스트 URL 열람 전용 공유 · 학년/종목 필터 · 일자별 경기결과 · 대진 개별 삭제 · 모바일 최적화 · Vitest 98건
...
### ✨ Features
- 대시보드 종목 카드에 종별 표기: 동일 종목 다학년/다성별 편성 구분 — `종목 (학년, 성별)` 형식 (예: 피구 (1학년, 여성))
  - 종목 그룹핑 키에 학년 포함(`종목__성별__학년`) — 기존에는 같은 종목·성별의 다학년 대진이 한 카드에 "N차 대진"으로 뭉쳐 구분 불가
  - 관리자 홈(LevelOverview)·게스트 화면(SportCard) 동일 적용, 학년 없음·혼성은 기존처럼 종목명만 표기
